...
2.  Para cada livro: `[ISBN] [Autor] & [Título]`.
...
* Ao final, mostra o total de "vitórias" de cada algoritmo (a busca interpolada vence em caso de empate) e a média de chamadas. 
...
* As palavras do dicionário e seus sinônimos são compostas por até 30 caracteres, exclusivamente em letras minúsculas.
...
Para cada consulta, exibe o percurso realizado na árvore (ex: `[elegante->nada->facil]`) e a lista de sinônimos da palavra encontrada. Caso a palavra não exista, o percurso indicará o ponto onde a busca falhou (ex: `[elegante->nada->trabalho->?]`).
...
* O mapeamento é feito a partir de um valor de chave gerado pelo texto da busca, usando um **checksum** de 8 bits para cada um dos caracteres. 
...
Exibe o servidor alocado para cada padrão de busca e o conteúdo atual do servidor. Mostra também as realocações quando um servidor está cheio (ex: `S0->S1`).
...
Este projeto desenvolve um sistema para apuração de resultados de loteria, identificando de forma eficiente as apostas com maior e menor número de acertos. [cite: 4390]

### 📄 Descrição do Problema

O sistema deve apurar os resultados de um concurso de loteria, premiando as faixas de maior e menor acertos. As regras são:
* Cada aposta consiste em 15 números, escolhidos de 1 a 50. [cite: 4391]
* O sorteio define 10 números distintos. [cite: 4392]
* O prêmio é dividido igualmente entre as apostas com o **maior número de acertos** e as com o **menor número de acertos**. [cite: 4391]
* O código de cada aposta é um número hexadecimal de 128 bits. 

### 💡 Estruturas de Dados Utilizadas

* **Heap Máximo (Max-Heap):** É uma árvore binária onde o valor de cada nó pai é maior ou igual ao valor de seus filhos. É usada aqui para gerenciar a faixa de "menor número de acertos". Ao manter um heap com os `k` menores acertos, o topo sempre conterá o **maior** valor dessa faixa, facilitando a comparação com novas apostas.
* **Heap Mínimo (Min-Heap):** É uma árvore binária onde o valor de cada nó pai é menor ou igual ao valor de seus filhos. É usada para gerenciar a faixa de "maior número de acertos". O topo sempre conterá o **menor** valor dessa faixa, permitindo uma comparação eficiente.
...
4.  Para cada aposta: `[Código Hexadecimal] [Número1] [Número2] ... [Número15]`.
...
Exibe as duas faixas de premiação no formato `[#Ganhadores : #Acertos : Prêmio Individual]`, seguido pela lista dos códigos das apostas vencedoras em cada faixa.
